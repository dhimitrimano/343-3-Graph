...
void testBonus() {
  ...
  g.connect("A", "C", 20);
  g.connect("D", "A", 100);
  assert((g.edgesSize() == 2) && "graph number of edges");
  g.connect("C", "A", 100);
  g.disconnect("C", "A");
  g.connect("B", "C", 200);
  g.connect("A", "D", 10);
  ...
  globalSS.str("");
  int testKruskal = g.mstKruskal("A", edgePrinter);
  string stringKruskal = globalSS.str();
  globalSS.str("");
  int testPrim = g.mstPrim("A", edgePrinter);
  string stringPrim = globalSS.str();
  assert(testKruskal != testPrim);
  assert(stringKruskal != stringPrim);

  g.connect("C", "A", 75);
  globalSS.str("");
  testKruskal = g.mstKruskal("A", edgePrinter);
  stringKruskal = globalSS.str();
  globalSS.str("");
  testPrim = g.mstPrim("A", edgePrinter);
  stringPrim = globalSS.str();
  assert(testKruskal == testPrim);
  assert(stringKruskal == stringPrim);
  if (!g.readFile("nonExistent.txt")) {
    return;
  }
}
